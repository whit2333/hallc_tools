...
void LH2_settings_rate() {
  ...
  for(auto& setting_group : csv::LH2_settings) { 
    ...
    for(auto& a_setting: setting_group.second) {
      ...
      auto plus_entry = ROOT::Experimental::Async([&a_setting,&use_LH2,&only_LH2](){
        ...
        entry0.rates.total_rate = entry0.rates.window_rate + entry0.rates.LH2_rate;
      ...
      auto Kplus_entry = ROOT::Experimental::Async([&a_setting,&table_entry0,&use_LH2,&only_LH2](){
        ...
        //  entry1.rate               = res_rate[3]*(entry1.Ibeam/50.0);
        //  entry1.sigma              = res_rate[4];
        //  entry1.time               = table_entry0[0].time;//entry1.counts/entry1.rate/(60.0*60.0);
        //  entry1.counts             = (60.0 * 60.0) * entry1.time * entry1.rate;
        //  entry1.Print();
        //  resulting_entries.push_back( entry1 );
        //}
        return resulting_entries;
      ...
      auto Kminus_entry = ROOT::Experimental::Async([&a_setting,&table_entry1,&use_LH2,&only_LH2](){
        std::vector<RunPlanTableEntry> resulting_entries;
        // pi minus
        RunPlanTableEntry entry0;
        entry0.kinematic = a_setting;
        ...
        //  entry1.rate               = res_rate[3]*(entry1.Ibeam/50.0);
        //  entry1.sigma              = res_rate[4];
        //  entry1.time               = table_entry1[0].time;//entry1.counts/entry1.rate/(60.0*60.0);
        //  entry1.counts             = (60.0 * 60.0) * entry1.time * entry1.rate;
        //  entry1.Print();
        //  resulting_entries.push_back( entry1 );
        //}
      ...
      run_plan_kaon_table.insert( run_plan_kaon_table.end(), table_entry2.begin(), table_entry2.end() );
      run_plan_kaon_table.insert( run_plan_kaon_table.end(), table_entry3.begin(), table_entry3.end() );
...

  {
    ...
    std::ofstream ofs("tables/LH2_run_plan_table.txt",std::ios_base::trunc);
    std::ofstream wiki_file("tables/LH2_run_plan_table.wiki",std::ios_base::trunc);
    ...
    std::ofstream json_ofile("tables/LH2_run_plan_table.json",std::ios_base::trunc);
  ...
  {
    double total_time = 0.0;
    std::ofstream ofs("tables/LH2_run_plan_kaon_table.txt",std::ios_base::trunc);
    std::ofstream wiki_file("tables/LH2_run_plan_kaon_table.wiki",std::ios_base::trunc);
    ...
    std::ofstream json_ofile("tables/LH2_run_plan_kaon_table.json",std::ios_base::trunc);
    json_ofile << TBufferJSON::ToJSON(&all_tables);
    json_ofile.close();
  }
}
...
std::vector<double> sidis_config_sigma_rate( RunPlanTableEntry& run_setting){
  ...
  double Loop1_LD2_Lumi = materials::ComputeLuminosityPerN({1, 2}, 
                                                 ...
                                                 10.0 /*cm*/, 
                                                 50.0e-6);
  double Loop1_LH2_Lumi = materials::ComputeLuminosityPerN({1, 1}, 
                                                 materials::LH2_density /*g/cm3*/,
                                                 10.0 /*cm*/, 
                                                 50.0e-6);
  ...
  std::vector<double> res = {
    targ_rates.LD2_XS, 
    targ_rates.LD2_rate, 
    targ_rates.total_LD2_rate,
    targ_rates.total_LH2_rate,
    targ_rates.LH2_XS,
    targ_rates.LH2_rate
  };
  return res;
