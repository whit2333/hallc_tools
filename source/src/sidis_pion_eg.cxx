...
void sidis_pion_eg(){
  ...
  HallCSetting hc_setting;

  //IPSV phi_HMS_psv( {-pi, pi }, "phi_hms");
  //IPSV phi_SHMS_psv({-pi, pi }, "phi_shms");
  ...
  //IPSV P_HMS_psv( {0.5,11.0},  "P_hms");
  //IPSV P_SHMS_psv({0.5,11.0}, "P_shms");
  ...
  PSV<Invariant> y_psv ({0.01,0.99}, Invariant::y, "y");
  PSV<Invariant> z_psv ({0.01,0.99}, Invariant::z, "z");
  PSV<Invariant> Q2_psv({0.5, 10.0}, Invariant::Q2, "Q2"); 
  IPSV phi_psv(      {-1.0*pi, pi}, "phi");
  IPSV phi_LAB_psv(  {-1.0*pi, pi}, "phi_e");
  IPSV PT_LAB_psv(   {0.0, 1.0}, "PT");
  ...
  auto diff_dxyzPTphiphie  = make_diff(x_psv, y_psv, z_psv, PT_LAB_psv, phi_psv, phi_LAB_psv);
  auto ps_dxyzPTphiphie    = make_phase_space(
      diff_dxyzPTphiphie,
  ...
  auto diff_xs_sidis       = make_diff_cross_section(ps_dxyzPTphiphie, SIDIS_xs_func);
  ...
  //auto sampler       = make_ps_sampler(make_integrated_cross_section(init_state, xs_spectrometers));

  auto fs_parts = make_final_state_particles({11, 211}, {0.000511, M_pion / GeV},
                                             Eprime_and_Ph_func);

  auto fs_SIDIS = make_final_state(ps_dxyzPTphiphie, fs_parts);
  ...
  //// --------------------------
  //
  std::cout << "Total cross section = " << total_XS/1000.0  << " ub\n";
  int    N_sim      = 100000;
