...
namespace hallc {
  namespace calibration {
    ...
    HMSCalorimeterCalibration::HMSCalorimeterCalibration(int rn) : run_number(rn) {}

    void HMSCalorimeterCalibration::Merge(const HMSCalorimeterCalibration& cc, double weight) {
      if( weight > 1.0 ) { 
        std::cerr << "error: weight > 1. Setting to 0.5. \n";
        weight = 0.5;
      }
    ...
    void HMSCalorimeterCalibration::MergeHelper(double w1, double w2, std::vector<double>& vec1,
                              const std::vector<double>& vec2) {
        for (int i = 0; i < vec1.size(); i++) {
          double g1 = vec1.at(i);
          double g2 = vec2.at(i);
    ...
    double HMSCalorimeterCalibration::GetGainCoeff(uint64_t block) const {
      if (block < Config::N_rows_preshower) {
        return neg_gain_cor.at(block);
      } else if (block < 2 * Config::N_rows_preshower) {
        return pos_gain_cor.at(block - Config::N_rows_preshower);
    ...
    void HMSCalorimeterCalibration::SetGainCoeffs(const std::array<double, fNpmts>& coeffs) {
      neg_gain_cor.clear();
      pos_gain_cor.clear();
      arr_gain_cor.clear();
      std::copy(coeffs.begin(), coeffs.begin() + fNrows_pr, std::back_inserter(neg_gain_cor));
    ...
    std::string HMSCalorimeterCalibration::PrepareJson() const {
      nlohmann::json j;
      j["thresholds"]["delta_low"]  = fDeltaMin;
      j["thresholds"]["delta_high"] = fDeltaMax;
      j["thresholds"]["beta_low"]   = fBetaMin;
    ...
    void HMSCalorimeterCalibration::LoadCalibration(int run_num, const std::string& fname) {
      LoadJsonCalibration(run_num, fname);
    }

    void HMSCalorimeterCalibration::LoadJsonCalibration(int rn, const std::string& fname) {
      // file checks assumed already complete.
      using nlohmann::json;
      json j_database;
      input_cal_file_name = fname;
    ...
    void HMSCalorimeterCalibration::WriteCalibration(int run_num, const std::string& fname) const {
      using nlohmann::json;
      nlohmann::json j_database;
      output_cal_file_name = fname;
      run_number           = run_num;
    ...
    void HMSCalorimeterCalibration::BuildTester(int run_num) const {
      nlohmann::json j;
      // nlohmann::json a_run;
      std::string a_run          = PrepareJson();
      j["0"]                     = nlohmann::json::parse(a_run);
    ...
    void HMSCalorimeterCalibration::ReadLegacyCalibration(const std::string& fname) {
      // Read in threshold parameters and initial gains.
      // File is assumed to have the following structure:
      // -10 22	Delta range, %
      // 0.5 1.5	Beta range
    ...
    void HMSCalorimeterCalibration::Print() const {
      std::ios_base::fmtflags f(std::cout.flags());
