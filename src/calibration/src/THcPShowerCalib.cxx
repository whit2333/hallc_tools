...
namespace hallc {
  namespace calibration {
    ...
    void THcPShowerCalib::Init() {
      ...
      fTree->SetBranchAddress("P.cal.pr.goodPosAdcPulseInt", P_pr_apos_p, &b_P_pr_apos_p);
      fTree->SetBranchAddress("P.cal.pr.goodNegAdcPulseInt", P_pr_aneg_p, &b_P_pr_aneg_p);
      fTree->SetBranchAddress("P.cal.fly.goodAdcPulseInt", P_sh_a_p, &b_P_sh_a_p);
      ...
      fTree->SetBranchAddress("P.cal.fly.nclust", &P_cal_fly_nclust, &b_P_cal_fly_nclust);
      fTree->SetBranchAddress("P.cal.fly.ntracks", &P_cal_fly_ntracks, &b_P_cal_fly_ntracks);

      // Histogram declarations.

      hEunc     = new TH1F("hEunc", "Edep/P uncalibrated", fEuncNBin, fEuncLoLo, fEuncHiHi);
      hEcal     = new TH1F("hEcal", "Edep/P calibrated", 200, 0., 2.);
      hDPvsEcal = new TH2F("hDPvsEcal", "#DeltaP versus Edep/P ", 400, 0., 2., 440, fDeltaMin - 1.,
                           fDeltaMax + 1.);
      hESHvsEPR = new TH2F("hESHvsEPR", "E_{SH} versus E_{PR}", 300, 0., 1.5, 300, 0., 1.5);
      ////
      hEPRunc       = new TH1F("hEPRunc", "EPR/P uncalibrated", 500, 0., 0.2);
      hETOTvsEPR    = new TH2F("hETOTvsEPR", "E_{TOT} versus E_{PR}", 300, 0., 1.5, 300, 0., 1.5);
      hETOTvsEPRunc = new TH2F("hETOTvsEPRunc", "E_{TOT} versus E_{PR} uncalibrated", 500, 0., 0.2,
                               500, 0., 0.2);
      hESHvsEPRunc =
          new TH2F("hESHvsEPRunc", "E_{SH} versus E_{PR} uncalibrated", 500, 0., 0.2, 500, 0., 0.2);
    ...
    void THcPShowerCalib::SolveAlphas() {
      ...
      cout << endl;
      cout << "Channels with hit number less than " << fMinHitCount << " will not be calibrated."
           << endl;
      cout << endl;
