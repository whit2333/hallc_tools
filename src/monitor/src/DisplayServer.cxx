...
namespace hallc {
  ...
  DisplayServer::DisplayServer(int http_port, int sock_srv_port )
      : _http_port(http_port),_sock_srv_port(sock_srv_port), 
        _server(std::make_shared<THttpServer>(
            (std::string("http:") + std::to_string(http_port) + std::string("?top=replay&thrds=2;rw"))
                .c_str())) {
    _server->SetCors();
    _mon = new TMonitor;
  }
