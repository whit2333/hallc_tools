...
class HallcEpics :
    ...
    def __init__(self):
        ...
        self.target_names = {2:"LH2 Loop 2 10cm",3:"LD2 Loop 3 10cm",5:"dummy"}
    ...
    def draw_menu(self,stdscr):
        ...
        while (k != ord('q')):
            ...
            win.addstr(1, 1, "             RUN {}  , setting {}".format(int(self.run_number), int(epics.pv.get_pv("hcRunSettingNumber").get())))
            win.addstr(2, 1, "      run time   : {:.2f} minutes".format(float(epics.pv.get_pv("hcCOINRunTime").get())/60.0 ))
            win.attroff(curses.A_BOLD)
            win.addstr(3, 1, "total run charge : {:.2f} mC".format(self.run_charge))
            win.addstr(4, 1, "setting charge   : {:.2f} mC".format(self.setting_charge))
            win.addstr(6, 1, "Beam Current     : {:.2f} uA, run avg. {:.2f} uA ".format(self.bcm1,self.beam_current))
            win.addstr(7, 1, "Target           : {}".format(self.target_names[self.target]))
            #win.refresh()
            # Refresh the screen
            stdscr.refresh()
            # Wait for next input
